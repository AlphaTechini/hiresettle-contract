...
impl HireSettleContract {
    ...
    // ----------------------------------------------------------
    // INTERNAL HELPER — per-company active count
    // ----------------------------------------------------------

    /// Decrement the per-company active engagement counter by 1, flooring at 0.
    fn decrement_company_active_count(env: &Env, company: &Address) {
        ...
        if count > 0 {
            env.storage()
                .persistent()
                .set(&DataKey::CompanyActiveCount(company.clone()), &(count - 1));
            env.storage().persistent().extend_ttl(
                &DataKey::CompanyActiveCount(company.clone()),
                100_000,
                6_300_000,
