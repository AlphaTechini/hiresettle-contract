...
fn create_engagement_with_referrer(
    env: &Env,
    client: &HireSettleContractClient,
    token_id: &Address,
    company: &Address,
...
// ============================================================
// get_referral_engagement_count
// ============================================================

#[test]
fn test_referral_engagement_count_empty() {
    let (env, contract_id, _token_id, _company, _recruiter, _arbiter) = setup();
    let client = HireSettleContractClient::new(&env, &contract_id);
    let referrer = Address::generate(&env);

    assert_eq!(client.get_referral_engagement_count(&referrer), 0);
}

#[test]
fn test_referral_engagement_count_matches_stored_referrer() {
    let (env, contract_id, token_id, company, recruiter, arbiter) = setup();
    let client = HireSettleContractClient::new(&env, &contract_id);
    let referrer = Address::generate(&env);
    let other_referrer = Address::generate(&env);

    create_engagement_with_referrer(
        &env,
        &client,
        &token_id,
        &company,
    ...
    assert_eq!(client.get_referral_engagement_count(&referrer), 2);
    assert_eq!(client.get_referral_engagement_count(&other_referrer), 1);
}

#[test]
fn test_referral_engagement_count_includes_cancelled_engagements() {
    let (env, contract_id, token_id, company, recruiter, arbiter) = setup();
    let client = HireSettleContractClient::new(&env, &contract_id);
    let referrer = Address::generate(&env);
    ...
    client.cancel_engagement(
        &company,
        &recruiter,
        &String::from_str(&env, "ENG-REF-CANCELLED"),
    );
